...
void InputSubsystem::init() {
    ...
    auto encoderConfigs = configuration_->getEncoderConfigs();
    auto buttonConfigs = configuration_->getButtonConfigs();
...
void InputSubsystem::configureEncoders(const std::vector<EncoderConfig>& encoderConfigs) {
    ...
    // Note: cette méthode n'est plus utilisée car les encodeurs sont configurés dans le constructeur
