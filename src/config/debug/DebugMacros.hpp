...
#define DEBUG_LOG(level, format, ...) \
    do { if (DEBUG_LEVEL >= level && isSerialReady()) { \
        char buffer[100]; \
...
// Niveau 2 : Stats détaillées
    ...
    // Débogage détaillé pour l'ordonnanceur de tâches (statistiques détaillées)
        ...
        #define DEBUG_SCHEDULER_VERBOSE(format, ...) DEBUG_LOG(DEBUG_SCHEDULER_LEVEL, "[SCHED-DETAIL] " format, ##__VA_ARGS__)
    #else
        #define DEBUG_SCHEDULER_VERBOSE(format, ...) do {} while(0)
    #endif
#else
