...
public:
     ...
     */
    const MidiControl* getMidiMappingForControl(ControlId controlId, ControlType controlType = ControlType::ENCODER_ROTATION) const;
...
private:
    ...
    std::unordered_map<uint32_t, std::size_t> midiMappingIndex;  // Pour accès rapide par ID composite (ID << 8 | type)
