...
ParameterWidgetMappingManager::ParameterWidgetMappingManager(const MappingConfig& config)
    : config_(config), initialized_(false) {
    reset();
}

void ParameterWidgetMappingManager::initializeMappings(
...
int8_t ParameterWidgetMappingManager::getWidgetIndexForCC(uint8_t cc_number) const {
    if (cc_number >= 128) {
        return -1;
    }
...
int8_t ParameterWidgetMappingManager::getWidgetIndexForButton(uint16_t button_id) const {
...
const std::map<uint16_t, uint8_t>& ParameterWidgetMappingManager::getButtonToWidgetMapping() const {
    return button_to_widget_mapping_;
}

const std::vector<ConfigurationMidiExtractor::ButtonInfo>&
ParameterWidgetMappingManager::getStandaloneButtons() const {
    return standalone_buttons_;
}

bool ParameterWidgetMappingManager::isInitialized() const {
    return initialized_;
}

void ParameterWidgetMappingManager::reset() {
...
ParameterWidgetMappingManager::MappingStats ParameterWidgetMappingManager::getStats() const {
    MappingStats stats;
...
void ParameterWidgetMappingManager::initializeCCMapping(
    const std::vector<ConfigurationMidiExtractor::MidiControlInfo>& midiControls) {
    ...
    for (const auto& control : midiControls) {
        if (widgetIndex >= config_.maxWidgets) {
            logDebug("Maximum widgets reached (" + String(config_.maxWidgets) +
                     "), skipping remaining controls");
            break;
        }
...
void ParameterWidgetMappingManager::initializeButtonMapping(
    const std::vector<ConfigurationMidiExtractor::MidiControlInfo>& midiControls,
    const std::vector<ConfigurationMidiExtractor::ButtonInfo>& buttonInfos) {
    logDebug("Initializing button → widget mapping");

    for (const auto& buttonInfo : buttonInfos) {
        logDebug("Processing button ID: " + String(buttonInfo.button_id) + ", Parent: " +
                 String(buttonInfo.parent_encoder_id) + ", Name: " + buttonInfo.name);
        ...
        if (buttonInfo.hasParent()) {
            ...
            bool found = false;
            for (size_t i = 0;
                 i < std::min(midiControls.size(), static_cast<size_t>(config_.maxWidgets));
                 i++) {
                if (midiControls[i].control_id == encoder_id) {
                    button_to_widget_mapping_[buttonInfo.button_id] = i;
                    logDebug("Mapped button " + String(buttonInfo.button_id) + " → widget " +
                             String(i) + " (encoder " + String(encoder_id) + ")");
                    found = true;
                    break;
                }
            }
            ...
            if (!found) {
                logDebug("Parent encoder " + String(encoder_id) + " not found for button " +
                         String(buttonInfo.button_id));
            }
        } else {
            // Bouton indépendant - l'ajouter à la liste des standalone
            standalone_buttons_.push_back(buttonInfo);
            logDebug("Added standalone button: " + String(buttonInfo.button_id) + " (" +
                     buttonInfo.name + ")");
        }
    }
...
void ParameterWidgetMappingManager::logInfo(const String& message) const {
    if (config_.enableLogging) {
        Serial.println("[ParameterWidgetMappingManager] " + message);
    }
}

void ParameterWidgetMappingManager::logDebug(const String& message) const {
    if (config_.enableLogging) {
        Serial.println("[ParameterWidgetMappingManager DEBUG] " + message);
    }
}