...
/**
 * @brief Mock de IMidiOut pour les tests
 */
class MockMidiOut : public IMidiOut {
public:
    MockMidiOut() 
        : noteOnCount_(0), noteOffCount_(0), ccCount_(0) {}
    ...
    // Méthode spécifique pour les tests (fournit le compteur de CC sous un nom compatible avec les tests existants)
    int getControlChangeCount() const { return ccCount_; }
    
    MidiChannel getLastChannel() const { return lastChannel_; }
    MidiNote getLastNote() const { return lastNote_; }
    uint8_t getLastVelocity() const { return lastVelocity_; }
    MidiCC getLastController() const { return lastController_; }
