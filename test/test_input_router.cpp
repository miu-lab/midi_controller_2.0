#include <unity.h>
#include "adapters/primary/input/InputRouter.hpp"
...
struct MockProfileManager : public IProfileManager
{
    ...
    {
        if (id == 1)
            return MidiControl{1, 10, true}; // pour EncoderTurnedEvent
        if (id == 2)
            return MidiControl{2, 60, false}; // pour ButtonPressed & Released
        return std::nullopt;
    }
